<?php
	...
	function curPlan($planNr){
		(isset($_GET['plan'])) ? $plan=$_GET['plan'] : $plan=1;
...
<div class="content-box blog-page-box">
    ...
    <table id="register-table">
		...
		<tr>
        	<td><label for="cemail">Confirm E-mail: </label></td>
        ...
        <tr>
        	<td colspan="2" class="subsectionTitle">
            	<input type="checkbox" name="business" id="business" /> I would like to register a Business name as a mail recipient at eSnail.ca
            </td>  
        </tr>        
        <tr>
        ...
        <tr>
        	<td colspan="2" class="subsectionTitle">
            	At eSnail we like to Save you Money, did you grab a promotional code?
            </td>  
        </tr>        
        <tr>
        ...
        <tr>
        	<td class="subsectionTitle">
            	<input type="checkbox" name="tos" id="tos" /> I have read and understood the <a href="/legal/" target="_blank">Terms of Service.</a>
            </td>
        ...
        <tr class="registerStep2">
        	<td colspan="2">
            	...
            	<select name="os0" id="plan">
                	<option value="1" <?php curPlan(1);?>>On Call - $2.00/mo</option>
                    <option value="6" <?php curPlan(6);?>>On Call - $20.00/year</option>
                    <option value="2" <?php curPlan(2);?>>Basic - $9.99/mo</option>
                    <option value="3" <?php curPlan(3);?>>Personal - $18.99/mo</option>
                    <option value="4" <?php curPlan(4);?>>Family - $21.99/mo</option>
        ...
        <tr>
        	<td class="subsectionTitle">
            	<input type="button" id="nextStep" value="Next Step!" class="order-button" />
            </td>
        </tr>         
    </table>
</div>
...
check = {
	...
	template: function(fieldId, status){
		...
		if(status==1) {	checkCell.html('<img src="/img/icons/tick.png">'); checkCell.attr('status', 1); }
			else { checkCell.html('<img src="/img/icons/exclamation.png">'); checkCell.attr('status', 0); }
	},
	...
	names: function(){
		...
		for(var field in fields) {
			...
			if($("#"+fieldId).val().length===0){ check.template(fieldId, 0); } else { check.template(fieldId, 1); }
		}
	},
